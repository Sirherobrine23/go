...
var (
	...
	_ fs.WriteAtFile   = (*openMapFile)(nil)
	_ fs.WriteSeekFile = (*openMapFile)(nil)
)
...
func (f *openMapFile) Write(b []byte) (int, error) {
	...
	if mapEntry.Mode.IsDir() {
		return 0, &fs.PathError{Op: "write", Path: f.path, Err: errors.New("is a directory")}
	}
	...
	n := len(b)
	neededLen := f.offset + int64(n)

	if neededLen > int64(cap(mapEntry.Data)) {
		newCap := neededLen
		// Basic growth strategy, similar to append
		if newCap < 2*int64(cap(mapEntry.Data)) && cap(mapEntry.Data) > 0 {
			newCap = 2 * int64(cap(mapEntry.Data))
		} else if newCap < 4096 && cap(mapEntry.Data) == 0 { // Initial small allocation
			newCap = 4096
		}
		if newCap < neededLen { // Ensure enough capacity
			newCap = neededLen
		}

		newData := make([]byte, len(mapEntry.Data), newCap)
		copy(newData, mapEntry.Data)
		mapEntry.Data = newData
	}

	if neededLen > int64(len(mapEntry.Data)) {
